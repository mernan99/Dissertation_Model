...
try:
    ...
    solver.maxsteps = 20000  # Allow more solver steps
    ...
    if __name__ == "__main__":
        ...
        # Filter valid outputs before Sobol analysis
        # Filter valid outputs before Sobol analysis
        valid_indices = ~np.isnan(ensemble_outputs).any(axis=0)
        filtered_outputs = ensemble_outputs[:, valid_indices]

        if filtered_outputs.shape[1] == 0:
            raise RuntimeError("No valid outputs for Sobol analysis after filtering.")
    ...
    def run_cardiovascular_model(params, t_τL, x):
        """
        Run the cardiovascular model for a single set of 9 parameters.
        Returns final values of pLV, psa, Vlv (as an example).
        """
        # Rebuild parameters for your model (τ_es, Δτ, Rmv, Zao, Rs, Csa, Csv, E_max, E_min)
        τ_es = params[0]
        Δτ = params[1]
        τ_ep = τ_es + Δτ
        ...
        try:
            # Simulate the model
            t_sol, y_sol, _ = solver.simulate(tfinal=x[-1], ncp_list=x)
        except Exception as e:
            print(f"Simulation failed for index {index} at time {x[-1]} with error: {e}")
            return None
        ...
        return np.array([pLV_final, psa_final, Vlv_final])

    marginals = [
        ot.Uniform(0.21, 0.34),      # τ_es
        ot.Uniform(0.15, 0.205),     # Δτ
        ot.Uniform(0.042, 0.078),    # Rmv
        ot.Uniform(0.0231, 0.0429),  # Zao
        ot.Uniform(0.777, 1.443),    # Rs
        ot.Uniform(0.791, 1.469),    # Csa
        ot.Uniform(7.7, 14.3),       # Csv
        ot.Uniform(1.05, 1.95),      # E_max
        ot.Uniform(0.021, 0.039)     # E_min
    ]
    distribution = ot.ComposedDistribution(marginals)

    N_train = 128
    X_train = np.array(distribution.getSample(N_train))
    simulation_end_time = 35
    t_τL = HRV(simulation_end_time)
    x = np.arange(0, simulation_end_time + 0.002, 0.002)
